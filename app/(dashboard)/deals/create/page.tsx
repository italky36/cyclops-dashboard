...
import type { DealRecipient } from '@/types/cyclops/deals';
...
export default function CreateDealPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.compliance_check_payments) {
        const hasWarnings = response.compliance_check_payments.some(p => !p.approved);
        if (hasWarnings) {
          setError('Сделка требует проверки комплаенс-службой. Проверьте детали сделки после создания.');
        }
      }
